<?php
/*
Plugin Name: MapSVG |  VestaThemes.com
...
function mapsvg_add_jscss_common(){
    ...
    if(MAPSVG_DEBUG){
	    ...
	    wp_register_script('mapsvg', MAPSVG_PLUGIN_URL . 'js/mapsvg.js', array('jquery'), (MAPSVG_RAND?rand():''));
...
function mapsvg_set_regions_table($map_id, $svg_file_path, $prefix){
    ...
    if(file_exists($important_stuff)){
        ...
        echo 'Debug info:<br />';
        echo '$_ROOT: '.$_SERVER['DOCUMENT_ROOT'];
	    echo '<br />';
        echo 'WP_CONTENT_DIR: '.WP_CONTENT_DIR;
	    echo '<br />';
        echo 'ABSPATH: '.ABSPATH;
	    echo '<br />';
        echo 'MAPSVG_MAPS_DIR: '.MAPSVG_MAPS_DIR;
	    echo '<br />';
        echo 'MAPSVG_MAPS_URL: '.MAPSVG_MAPS_URL;
	    echo '<br />';
        echo 'MAPSVG_MAPS_UPLOADS_DIR: '.MAPSVG_MAPS_UPLOADS_DIR;
	    echo '<br />';
        echo 'MAPSVG_MAPS_UPLOADS_URL: '.MAPSVG_MAPS_UPLOADS_URL;
	    echo '<br />';
        echo 'ABSPATH: '.ABSPATH;
	    echo '<br />';
...
//                $searchable_fields = explode(',',$searchable_fields);
    ...
    $query = 'SELECT *'.$region_title.$select_distance.' FROM '.mapsvg_table_name($map_id, $table).'
    '.$filter_regions.'  
    '.$filters_sql.'    
    '.$having.'     
...
';
        ...
        echo MAPSVG_PLUGIN_URL."gm_download.php?file=".MAPSVG_MAPS_UPLOADS_URL . $filename;
        die();
...
function mapsvg_geocoding ($address, $return_as_array = true) {
    ...
    if($google_geocode_api_key) {
	    ...
	    $data    = url_get_contents( 'https://maps.googleapis.com/maps/api/geocode/json?key=' . $google_geocode_api_key . '&address=' . $address . '&sensor=true&language=' . $lang);
	    if ( $data ) {
		    ...
		    $response = array( 'status'        => 'CONNECTION_ERROR',
		                       'error_message' => 'Can\'t connect to Google Geocoding API'
		    );
	    }
    } else {
...
function url_get_contents ($url) {
    if (function_exists('curl_exec')){
        ...
        $url_get_contents_data = json_encode(array('status'=>'PHP_SETTINGS_ERROR','error_message'=>'Can\'t connect to remote server. Please enable "allow_url_fopen" option in php.ini settings or install cURL.'));
    }
